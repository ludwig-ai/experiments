...
def load_porto_seguro_safe_driver():
    train_df = porto_seguro_safe_driver.load()
    ...
    val_df['split'] = 1
    test_df['split'] = 2
    porto_seguro_safe_driver_df = pd.concat([train_df, val_df, test_df], ignore_index=True)
    return porto_seguro_safe_driver_df
