...
def load_connect4():
    ...
    if "split" not in connect4_df.columns:
        connect4_df["split"] = 0
    train_df = connect4_df[connect4_df["split"] == 0]
    val_df = connect4_df[connect4_df["split"] == 1]
    test_df = connect4_df[connect4_df["split"] == 2]
    ...
    connect4_df = pd.concat([train_df, val_df, test_df], ignore_index=True)
    return connect4_df
